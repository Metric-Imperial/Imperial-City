-- Imperial City copy of qbx_properties/config/shared.lua.
--
-- The recipe copies this OVER the downloaded resource on every deploy, which is
-- the only redeploy-safe way to register an interior: qbx_properties is a
-- third-party resource, so an edit made directly to it is wiped the next time
-- that resource is downloaded. This is the same pattern the recipe already uses
-- for ox_inventory's items.lua and shops.lua.
--
-- Keep this in step when qbx_properties updates. It is a WHOLE-FILE override, so
-- any upstream change to the rest of the file is lost until it is merged in
-- here. The only imperial addition is the imperial_furnished_motel_room entry in
-- the interiors table below.
...
return {
    ...
    interiors = {
        -- Imperial furnished motel room. A prop shell, not an MLO: the
        -- wardrobes, dresser, bed and door are baked into the model, and
        -- imperial_shells spawns the wall art and pendants on top of it.
        --
        -- Offsets are from the shell origin, which is the centre of its floor.
        -- They are COMPUTED by _work/mlo/shell/build_shell.py from the same
        -- constants that position the furniture, so they cannot drift out of
        -- alignment with it. Do not hand-edit: re-run that script and paste what
        -- it prints.
        [`imperial_furnished_motel_room`] = {
            exit     = vec3(3.20, 0.00, 0.0),   -- inside the door on the +X wall
            clothing = vec3(-2.20, 0.91, 0.0),  -- in front of the wardrobe run
            stash    = vec3(1.80, 1.12, 0.0),   -- in front of the dresser
            logout   = vec3(1.65, -1.50, 0.0),  -- beside the bed
        },
        [`furnitured_midapart`] = {
